...
#endif
#else
#define _RPT5(rptno, msg, a1, a2, a3, a4, a5) _RPT_BASE((rptno, NULL, 0, NULL, msg, a1, a2, a3, a4, a5))
#endif
...
BOOL APIENTRY DllMain(HMODULE hModule, ULONG ulReason, LPVOID lpReserved) {
...
*/
			...
			catch (AvisynthError error) 
			{
				...
				catch (AvisynthError) {filter_graph = 0;}
			}
...
/*
//ATTMEPT TO CACHE AUDIO PROPERLY
    ...
    try {
      ...
      try {
        ...
        if (return_val.IsClip()) {
          ...
          if ((env->GetMTMode(false) > 0) && (env->GetMTMode(false) < 5)) {
				filter_graph = new CacheMT1(filter_graph, env);	
			  filter_graph = new Distributor(filter_graph,env);
...
//The functoin takes paramaters as long ptr's, not longs!!
												  ...
												  struct _CONTEXT *ContextRecord, void * DispatcherContext)
{
  ...
  if (ExceptionRecord->ExceptionFlags == 0)	{  // First pass?
	...
	(((struct Est_Frame *)EstablisherFrame)->retarg)[1] = (INT_PTR)ExceptionRecord->ExceptionAddress;
...
void CAVIStreamSynth::ReadHelper(void* lpBuffer, int lStart, int lSamples, unsigned code[4]) {
  ...
  void* handler = _Exp_except_handler2;
#ifndef _AMD64_
