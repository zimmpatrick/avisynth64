...
void __stdcall C_VideoFilter::SetCacheHints(int cachehints, int frame_range) 
{
	if (d.set_cache_hints) {
		...
		if (cachehints == 0x8666 && frame_range && *(int*)frame_range == (int)(INT_PTR)&d) // 0x8666 = Cache::GetMyThis
			*(int*)frame_range = (int)(INT_PTR)this;
		if (d.error)
			throw AvisynthError(d.error);
	}
...
//Function courtesy of kemuri-_9, doom9 and doom10 forums
AVSValue __cdecl load_c_plugin(AVSValue args, void * user_data, IScriptEnvironment * env)
{
	const char * filename = args[0].AsString();
	HMODULE plugin = LoadLibrary(filename);
	if (!plugin)
		env->ThrowError("Unable to load C Plugin: %s", filename);
    AvisynthCPluginInitFunc func = 0;
#ifndef AVSC_USE_STDCALL //--> compiler will remove any references to specific function calling conventions . . . left in just in case (if it isn't broken . . .)
    func = (AvisynthCPluginInitFunc)GetProcAddress(plugin, "avisynth_c_plugin_init");
#else // AVSC_USE_STDCALL
    func = (AvisynthCPluginInitFunc)GetProcAddress(plugin, "avisynth_c_plugin_init@4");
    if (!func)
