...
/*#else
  ...
  if (n>=minframe && n<=maxframe) { // Is this frame in the range we are tracking
	...
	if (++miss_count > MAX_CACHE_MISSES) {
	  ...
	  miss_count = MIN_INT; // Hugh negative
	}
  } // if (n>=minframe ... else
...
/*
This hack seems needless with current compilers
#ifndef _AMD64_
  __asm mov ebx,ebx  // Hack! prevent compiler from trusting ebx contents across call
#else
  __asm mov rbx,rbx  // Hack! prevent compiler from trusting ebx contents across call
...
*/
  ...
  // a 2nd filter is hitting the cache outside the radius
...
void __stdcall Cache::SetCacheHints(int cachehints,int frame_range) {
  ...
  if ((cachehints == GetMyThis) && (frame_range != 0)) {
	*(int *)frame_range = (INT_PTR)(void *)this;
	return;
  }
...
AVSValue __cdecl Cache::Create_Cache(AVSValue args, void*, IScriptEnvironment* env) 
{
  ...
  if (p) {
	...
	p->SetCacheHints(GetMyThis, (INT_PTR)&q);
