...
void memcpy_amd(void *dest, const void *src, size_t n)
{
	...
	mov		rcx, [n]		; number of bytes to copy
	mov		rdi, [dest]		; destination
	mov		rsi, [src]		; source
	mov		rbx, rcx		; keep a copy of count
...
$memcpy_do_align:
	...
	sub		rbx, rcx		; update copy count
	neg		rcx				; set up to jump into the array
	lea		r9, [$memcpy_align_done]
	add		rcx,r9
...
$memcpy_align_done:			; destination is dword aligned
	mov		rcx, rbx		; number of bytes left to copy
...
$memcpy_ic_2:
	mov		rcx, rbx		; has valid low 6 bits of the byte count
$memcpy_ic_3:
	shr		rcx, 2			; dword count
	and		rcx, 1111b		; only look at the "remainder" bits
	neg		rcx				; set up to jump into the array
	lea		r9, [$memcpy_last_few]
	add		rcx,r9
...
$memcpy_last_few:		; dword aligned from before movsd's
	mov		rcx, rbx	; has valid low 2 bits of the byte count
...
$memcpy_final: 
	emms				; clean up the MMX state
	sfence				; flush the write buffer
	mov		rax, [dest]	; ret value = destination pointer
