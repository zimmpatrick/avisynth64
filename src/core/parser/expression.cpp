...
void ExpFunctionCall::InsertCache(AVSValue &result,AVSValue *args,IScriptEnvironment* env,bool implicit_last)
{
  ...
  result.AsClip()->SetCacheHints(Cache::GetMyThis, (int)&q);

	// Do not cache another cache!
  if (q == (int)(void *)result.AsClip())return;

int mtmode=env->GetMTMode(false);
  env->SetMTMode(-5,0,false);//restore mtmode
  switch(mtmode)
  {
    case -1:
    {
    ...
    case 0:
    {
      result=new Cache(result.AsClip(), env);
      break;
    }
    case 1:
    {
    ...
    case 2:
    {
      ...
      filters[0]=result;
	  //creates as many instances that there are threads
