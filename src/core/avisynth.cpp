...
void* VideoFrame::operator new(size_t) {
  ...
  result->next = (LinkedVideoFrame*)InterlockedExchangePointer(&g_Bin->g_VideoFrame_recycle_bin,result);//g_VideoFrame_recycle_bin is assigned twice to result->next just in case another thread reads result->next before the last assignment 
  return &result->vf;
}
...
ScriptEnvironment::ScriptEnvironment()
  ...
  try {
	...
	//::TODO:: CHANGE ME TO CLEANER CODE
	//Changed to PDWORD_PTR's because that's the params the function takes, and the winsdk is silly
	//PDWORD_PTR={ULONG_PTR={unsigned __int64} *} *
	PDWORD_PTR systemaffinitymask = new DWORD_PTR;
    PDWORD_PTR processaffinitymask= new DWORD_PTR;
    GetProcessAffinityMask(GetCurrentProcess(),processaffinitymask,systemaffinitymask);
    do//count the number of bits set = number of logical procesors available for avisynth
      if((DWORD_PTR)processaffinitymask&1)
        nthreads++;
    while(processaffinitymask = (PDWORD_PTR)((DWORD_PTR)processaffinitymask>>1));
...
bool ScriptEnvironment::SetVar(const char* name, const AVSValue& val) {
  ...
  if(mt_mode>0)  {
	  if(ScriptEnvironmentTLS* tls=(ScriptEnvironmentTLS*)TlsGetValue(TlsIndex))  {
			if(!tls->vartable)
				tls->vartable=new VarTable(var_table,var_table);
...
#endif
  const int offset = (-(INT_PTR)(vfb->GetWritePtr())) & (FRAME_ALIGN-1);  // align first line offset
...
PVideoFrame __stdcall ScriptEnvironment::NewVideoFrame(const VideoInfo& vi, int align) {
  ...
  if (vi.IsPlanar()) { // Planar requires different math ;)
    ...
    retval=ScriptEnvironment::NewPlanarVideoFrame(vi.width, vi.height, align, !vi.IsVPlaneFirst());  // If planar, maybe swap U&V
  } else {
...
bool ScriptEnvironment::MakeWritable(PVideoFrame* pvf) {
  ...
  InterlockedDecrement(&dst->vfb->refcount);//After the VideoFrame has been assigned to a PVideoFrame it is safe to decrement the refcount (from 2 to 1)
    ...
    for (i = video_frame_buffers.prev; i != &video_frame_buffers; i = i->prev) {
      if (InterlockedCompareExchange(&i->refcount,1,0)==0) 
      {
        if (i->GetDataSize() == size) 
        {
          ++g_Mem_stats.PlanB;
          if(j)
            InterlockedDecrement(&j->refcount);
          InterlockedIncrement(&i->sequence_number);  // Signal to the cache that the vfb has been stolen
          return i;
        }
        if (i->GetDataSize() > size) 
