...
void __stdcall MergeChannels::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) 
{
	...
	for (i = 0;i < num_children;i++) 
	{
		...
		switch (bpcc) 
		{
			...
			case 4: 
				// mono float/32 bit, stereo 16 bit
				for (int l = 0, k=dst_offset; l < count; l++, k+=bps)
			...
			case 8: 
				...
				//if (env->GetCPUFlags() & CPUF_MMX) 
				{
					__asm 
					{
						...
						align 16
					label:
						movq	mm0,[rax]    ; do pairs of quads
						movq	mm1,[rax+8]
						add		rax,16
						movq	[rdi],mm0
						movq	[rdi+rdx],mm1
			...
			default: 
				// everything else, 1 byte at a time
...
void __stdcall Amplify::GetAudio(void* buf, __int64 start, __int64 count, IScriptEnvironment* env) 
{
	...
	if (vi.SampleType() == SAMPLE_INT16) 
	{
		...
		iloop0:
			xor		rsi, rsi					; j
		jloop0:
			mov		eax, DWORD PTR [rcx+rsi*4]	; i_v[j]
			movsx	edx, WORD PTR [rdi]		; *samples
			inc		esi						; j++
			imul	edx
			add		edi, 2						; samples++
			add		eax, 65536
	...
	if (vi.SampleType() == SAMPLE_INT32) 
	{
		...
		notpossat1:
			shrd	eax, edx, 17				; n>>17
		saturate1:
			mov		DWORD PTR [rdi-4], eax		; *samples
			cmp		esi, [channels]			; j < channels
			jl		jloop1
...
saturate2:
    ...
    __asm 
	{
          ...
          align  16
iloop2:
          movsx	 eax, WORD PTR [rdi]		; *samples
          imul	 ecx
          add	 edi, 2						; samples++
          add	 eax, 65536
          adc	 edx, 0
...
iloop3:
		  ...
		  mov    ebx, eax
		  mov    ecx, edx
          movsx	 eax, WORD PTR [rsi]		; *clip_samples
          add	 esi, 2						; clip_samples++
          imul	 [t2_factor]
          add	 eax, ebx
          adc	 edx, ecx
	...
	if (vi.IsSampleType(SAMPLE_INT16)) 
	{
		...
		//if (env->GetCPUFlags() & CPUF_MMX) 
		{
			...
			__asm 
			{
				...
				mov        rax, this
				punpckldq  mm5, mm5             ; 00002000 00002000
				movd       mm4, [rax].mLpScl    ; 00000000 LpScl
				movd       mm3, [r_NLpScl]      ; 1 << (NLpScl-1)
				punpckldq  mm4, mm4             ; LpScl | LpScl
				movd       mm2, [rax].mNhg      ; Number of guard bits
				punpckldq  mm3, mm3             ; 00001000 00001000
			}
#endif
			...
			while (dst < dst_end) 
			{
				...
				{ // do 2 channels at once
					...
					__asm 
					{
						...
						packssdw	mm7, mm7              ; pack with signed saturation ready for output
						mov			[dst], rax
						movd		[rax-4], mm7          ; deposit 2 output samples
						jmp			done1
						align 16
					dosingle:
						psrad		mm7, NLpScl           ; strip guard bits
						...
						mov			[dst], rax
						movd		edx, mm7
						mov			[rax-2], dx           ; deposit 1 output sample
						align 16
					done1:
					}
#endif
				...
				{ // Don't be a creep ;-)
					mov		rdx, this
					mov		eax, dtberror            ; time increment error accumulator
					mov		ecx, [rdx].dtb           ; time increment
					add		eax, [rdx].dtbe          ; add error per cycle
					mov		edx, dword ptr [pos+4]
					cmp		eax, 0x80000000          ; accumulated 1 full error yet?
					jb		nofix
					inc		ecx                      ; += 1
...
void FilterUD_mmx(short *Xp, unsigned Ph, int _inc, int _dhb, short *p_Imp, unsigned End) 
{
	...
	if (_inc > 0) 
	{						// If doing right wing drop extra coeff, so when Ph is
...
void FilterUD_mmx(short *Xp, unsigned Ph, int _inc, int _dhb, short *p_Imp, unsigned End) 
{
	//josh: todo: check if unsigned's being passed through here were meant as pointers
	unsigned Ho = (Ph * (unsigned)_dhb) >> Np;
	...
	__asm 
	{
		...
		mov			rdx,rsi				; Fold into end for improved pairing
		mov			eax,Amask
		shr			edx,Na				; Ho >> Na
		and			eax,esi				; Ho & Amask
		cmp			edx,[End]
		movd		mm1,Amasktab[rax*4]	; 0000 0000 eax 128-eax
		jae			donone

		align		16
	loop1:
		movd		mm0,[rcx+rdx*2]		; 0000 0000 Imp[Ho>>Na7+1] Imp[Ho>>Na7]
		add			esi,[_dhb]			; Ho += dhb
		pmaddwd		mm0,mm1				; 00000000 Imp[h+1]*a + Imp[h]*(128-a)
		movd		mm1,[rdi]			; 0000 0000 *(Xp+1) *Xp
		paddd		mm0,mm6				; += round
		add			edi,[_inc]			; Xp += Inc
		pslld		mm0,16-Na			; <<= 16-Na
		mov			eax,Amask
