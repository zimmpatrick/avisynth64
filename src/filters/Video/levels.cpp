...
#ifndef _AMD64_
void __declspec(naked) asm_tweak_ISSE_YUY2( BYTE *srcp, int w, int h, int modulo, __int64 hue, __int64 satcont, __int64 bright) 
{
...
void __declspec(naked) asm_tweak_ISSE_YUY2( BYTE *srcp, int w, int h, int modulo, __int64 hue, __int64 satcont, __int64 bright) 
{
	...
	__asm 
	{
		...
		pxor		mm0, mm0
		movq		mm1, norm				// 128 16 128 16
		movq		mm2, [rsp+32+24]		// Cos -Sin Sin Cos (fix12)
		movq		mm3, [rsp+32+32]		// Sat Cont Sat Cont (fix9)
		movq		mm4, mm1
		paddw		mm4, [rsp+32+40]		// 128 16+Bright 128 16+Bright

		mov			rsi, [rsp+32+8]			// srcp
		mov			edx, [rsp+32+16]		// height
	y_loop:
		mov			ecx, [rsp+32+12]			// width
	x_loop:
		movd		mm7, [rsi]   			// 0000VYUY
		punpcklbw	mm7, mm0
		psubw		mm7, mm1				//  V Y U Y
		pshufw		mm6, mm7, 0xDD			//  V U V U
		pmaddwd		mm6, mm2				// V*Cos-U*Sin V*Sin+U*Cos (fix12)
		...
		add			rsi, [rsp+16+20]		// skip to next scanline (add modulo)
		dec			edx
		jnz			y_loop
...
Limiter::Limiter(PClip _child, int _min_luma, int _max_luma, int _min_chroma, int _max_chroma, int _show, IScriptEnvironment* env)
    : GenericVideoFilter(_child),
	CodeGenerator(false), //x64: Codegenerator construtor takes arg x64
  min_luma(_min_luma),
  max_luma(_max_luma),
...
PVideoFrame __stdcall Limiter::GetFrame(int n, IScriptEnvironment* env) 
{
	...
	{
		if (show == show_luma) 
		{  // Mark clamped pixels red/yellow/green over a colour image
			for (int y = 0; y < height; y++) 
			{
				for (int x = 0; x < row_size; x+=4) 
				{
		...
		else if (show == show_luma_grey) 
		{    // Mark clamped pixels coloured over a greyscaled image
			for (int y = 0; y < height; y++) 
			{
				for (int x = 0; x < row_size; x+=4) 
				{
		...
		else if (show == show_chroma) 
		{    // Mark clamped pixels yellow over a colour image
			for (int y = 0; y < height; y++) 
			{
				for (int x = 0; x < row_size; x+=4) 
				{
...
DynamicAssembledCode Limiter::create_emulator(int row_size, int height, IScriptEnvironment* env) {
  ...
  //x64: Constructor for assembler takes bool argument x64
