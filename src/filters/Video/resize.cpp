...
void VerticalReduceBy2::mmx_process(const BYTE* srcp, int src_pitch, int row_size, BYTE* dstp, int dst_pitch, int height) 
{
	...
	if ((row_size&3)==0) 
	{  // row width divideable with 4 (one dword per loop)
		__asm 
		{
...
#define R_SRC rdx
#define R_DST rdi
#define R_XOFFSET rax
#define R_YLEFT rbx
#define R_SRC_PITCH rcx
#define R_DST_PITCH rsi
//josh: todo: come back and see if we can't disambiguate these ref's a bit
...
void HorizontalReduceBy2::isse_process_yuy2(PVideoFrame src,BYTE* dstp, int dst_pitch) {
	 ...
	 **/
	if ((row_size&3)==0) 
	{  // row width divideable with 8 (one qword per loop)
		__asm 
		{
			movq mm6,[zero_mask];
			movq mm7,[inv_0_mask];
...
void HorizontalReduceBy2::isse_process_yuy2(PVideoFrame src,BYTE* dstp, int dst_pitch) {
	...
	const BYTE* srcp = src->GetReadPtr();
	const __int64 src_pitch = src->GetPitch();
	__int64 row_size = ((src->GetRowSize())>>1)-4;
	__int64 _dst_pitch = (__int64) dst_pitch;

	const __int64 height = vi.height;
	__int64 yleft = height;
