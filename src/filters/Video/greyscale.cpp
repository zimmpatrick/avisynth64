...
yloop:
		...
		test		rsi,0x7					; qword aligned?
		jz			xloop1					; yes

		movd		mm5,[rsi]				; process 1 dword
		pand		mm5,mm7					; keep luma
		add			rsi,4					; srcp++
		por			mm5,mm6					; set chroma = 128
		dec			ecx						; count--
		movd		[rsi-4],mm5				; update 2 pixels
...
xloop1:
		...
		movq		mm0,[rsi+0]				; process 1 qword
		movq		mm1,[rsi+8]
		pand		mm0,mm7					; keep luma
		pand		mm1,mm7
		por			mm0,mm6					; set chroma = 128
		por			mm1,mm6
		movq		[rsi+0],mm0				; update 2 pixels
		movq		[rsi+8],mm1
...
xlend1:
		...
		align		16
xloop3:
		movq		mm0,[rsi]				; process 1 qword
		 sub		ecx,2					; count-=2
		pand		mm0,mm7					; keep luma
		 add		rsi,8					; srcp+=2
		por			mm0,mm6					; set chroma = 128
		 cmp		ecx,2					; any qwords left
		movq		[rsi-8],mm0				; update 4 pixels
		 jge		xloop3					; more qwords ?
...
xlend2:
		...
		movd		mm0,[rsi]				; process 1 dword
		pand		mm0,mm7					; keep luma
		por			mm0,mm6					; set chroma = 128
		movd		[rsi-4],mm0				; update 2 pixels
		add			rsi,4					; srcp++
...
xlend3:
  ...
  //else if (vi.IsRGB32() && (env->GetCPUFlags() & CPUF_MMX)) {
    ...
    __asm {
		mov			rdi,srcp
		mov			r10d,pitch			;store pitch in r10 (volatile) and not used in function calls
		pcmpeqd		mm1,mm1
		pxor		mm0,mm0
		psrlq		mm1,63				; 0x0000000000000001
		pcmpeqd		mm3,mm3
		...
		align		16
rgb2lum_mmxloop:
		movq		mm6,[rdi+rcx*4]		; Get 2 pixels
		movq		mm4,mm3				; duplicate alpha mask
		movq		mm5,mm6				; duplicate pixels
		pand		mm4,mm6				; extract alpha channel [ha00000la000000]
		punpcklbw	mm6,mm0				; [00ha|00rr|00gg|00bb]		-- low
		...
		por			mm6,mm4				; [ha|hg|hg|hg|la|lg|lg|lg]
		cmp			ecx,edx				; loop >= myx
		movq		[rdi+rcx*4-8],mm6	; update 2 pixels
		jnge		rgb2lum_mmxloop

		test		edx,1				; Non-mod 2 width
		jz			rgb2lum_even

		movd		mm6,[rdi+rcx*4]		; Get 1 pixels
		movq		mm4,mm3				; duplicate alpha mask
		pand		mm4,mm6				; extract alpha channel [xx00000la000000]
		punpcklbw	mm6,mm0				; [00ha|00rr|00gg|00bb]
		pmaddwd		mm6,mm2				; [0*a+cyr*r|cyg*g+cyb*b]
		...
		psrld		mm6,8				; [00|xx|xx|xx|00|lg|lg|lg]
		por			mm6,mm4				; [xx|xx|xx|xx|la|lg|lg|lg]
		movd		[rdi+rcx*4],mm6	; update 1 pixels

rgb2lum_even:
		add			rdi,r10				;r10d contains pitch
		mov			edx,myx
		xor			ecx,ecx
		dec			eax
		jnle		rgb2lum_mmxloop
