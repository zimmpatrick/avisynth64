...
AVSValue __cdecl Dissolve::Create(AVSValue args, void*, IScriptEnvironment* env) 
{
  ...
  const double fps = args[3].AsFloat(24);
  PClip result = args[0].AsClip();
  for (int i=0; i < args[1].ArraySize(); ++i)
...
PVideoFrame Dissolve::GetFrame(int n, IScriptEnvironment* env) 
{
  ...
  if ( (!(a->GetRowSize(PLANAR_Y_ALIGNED)&7)) ) {  // MMX and Video is mod 8  
...
{
  return new AudioDub(args[0].AsClip(), args[1].AsClip(), (INT_PTR)mode, env);
}
...
PClip __cdecl ColorClip(PClip a, int duration, int color, double fps, IScriptEnvironment* env) {
