...
PVideoFrame __stdcall MergeChroma::GetFrame(int n, IScriptEnvironment* env)
{
  ...
  if (weight<0.9999f) {
    if (vi.IsYUY2()) {
	  ...
	  if ((weight>0.4999f) && (weight<0.5001f)) {
...
void mmx_merge_luma( unsigned int *src, unsigned int *luma, int pitch, int luma_pitch,int width, int height )
{
	...
	for (int y=0;y<height;y++) 
	{
		...
		__asm 
		{
			push	rbx // bloody compiler forgets to save ebx!!
			mov		rax,src
			xor		rcx,rcx
			mov		rbx,luma
			align 16
		goloop:
			...
			; Processes 8 pixels at the time
			movq	mm0,[rax+rcx]		; chroma 4 pixels
			movq	mm1,[rax+rcx+8]  ; chroma next 4 pixels
			pand	mm0,mm6
			movq	mm2,[rbx+rcx]  ; load luma 4 pixels
			pand	mm1,mm6
			movq	mm3,[rbx+rcx+8]  ; load luma next 4 pixels
			pand	mm2,mm7
			pand	mm3,mm7
			por		mm0,mm2
			por		mm1,mm3
		...
		outeven:
			...
			movd	mm0,[rax+rcx]		; chroma 2 pixels
			movd	mm2,[rbx+rcx]  ; load luma 2 pixels
			pand	mm0,mm6
			pand	mm2,mm7
			por		mm0,mm2
			movd	[rax+rcx],mm0
...
void mmx_weigh_luma(unsigned int *src,unsigned int *luma, int pitch, int luma_pitch,int width, int height, int weight, int invweight)
{
	...
	for (int y=0;y<height;y++) 
	{
		...
		__asm 
		{
			...
			cmp			ecx,[lwidth_bytes]		; Is eax(i) greater than endx
			jge			outloop					; Jump out of loop if true
			movq		mm3,[rax+rcx]			; original 4 pixels   (cc)
			align 16
		goloop:
			; Processes 4 pixels at the time
			movq		mm2,[rbx+rcx]			; load 4 pixels
			movq		mm1,mm3					; move original pixel into mm3
			punpckhwd	mm3,mm2					; Interleave upper pixels in mm3 | mm3= CCLL ccll CCLL ccll
			movq		mm0,mm1
			punpcklwd	mm1,mm2					; Interleave lower pixels in mm1 | mm1= CCLL ccll CCLL ccll
		...
		outloop:
			...
			cmp			ecx,[row_size]
			jge			exitloop
			movd		mm1,[rax+rcx]			; original 2 pixels
			movd		mm2,[rbx+rcx]			; luma 2 pixels
			movq		mm0,mm1
			punpcklwd	mm1,mm2					; mm1= CCLL ccll CCLL ccll
			pand		mm0,mm6					; mm0= 0000 0000 cc00 cc00
			pand		mm1,mm7					; mm1= 00LL 00ll 00LL 00ll
...
void mmx_weigh_chroma( unsigned int *src,unsigned int *chroma, int pitch, int chroma_pitch,int width, int height, int weight, int invweight )
{
	...
	for (int y=0;y<height;y++) 
	{
		...
		__asm 
		{
			...
			jge			outloop					; Jump out of loop if true
			; Processes 4 pixels at the time
			movq		mm1,[rax+rcx]			; original 4 pixels   (cc)
			movq		mm2,[rbx+rcx]			; load 4 pixels
			align 16
		goloop:
			movq		mm3,mm1
		...
		outloop:
			...
			cmp			ecx,[row_size]
			jge			exitloop
			movd		mm0,[rax+rcx]			; original 2 pixels
			movd		mm2,[rbx+rcx]			; luma 2 pixels
			movq		mm1,mm0
			punpcklwd	mm1,mm2					; mm1= CCLL ccll CCLL ccll
			psrlw		mm1,8					; mm1= 00CC 00cc 00CC 00cc
...
void mmx_weigh_plane(BYTE *p1, const BYTE *p2, int p1_pitch, int p2_pitch,int rowsize, int height, int weight, int invweight) 
{
	...
	//josh: just chnaged these to int64 b/c they're used to adjust pointers
	__int64 _p1_pitch = (__int64) p1_pitch;
	__int64 _p2_pitch = (__int64) p2_pitch;
	...
	__asm 
	{
		...
		mov			rsi,[p1]
		mov			rdi,[p2]
		xor			rcx, rcx			// rcx = ecx = Height counter
		mov			edx,[height]
		test		ebx, ebx
		jz			outy
	...
	yloopback:
		...
		align 16
testloop:
		movq		mm0,[rdi+rax]  // y7y6 y5y4 y3y2 y1y0 img2
		movq		mm1,[rsi+rax]  // Y7Y6 Y5Y4 Y3Y2 Y1Y0 IMG1
		movq		mm2,mm0
		punpcklbw	mm0,mm1        // Y3y3 Y2y2 Y1y1 Y0y0
		punpckhbw	mm2,mm1        // Y7y7 Y6y6 Y5y5 Y4y4
		movq		mm1,mm0        // Y3y3 Y2y2 Y1y1 Y0y0
...
// mm0 mm1
	...
	yloopback:
		...
		align 16
	testloop:
		movq	mm0,[rdi+rax-16]  // y7y6 y5y4 y3y2 y1y0 img2
		movq	mm1,[rdi+rax- 8]  // yFyE yDyC yByA y9y8 img2
		pavgb	mm0,[rsi+rax-16]  // Y7Y6 Y5Y4 Y3Y2 Y1Y0 IMG1
		pavgb	mm1,[rsi+rax- 8]  // YfYe YdYc YbYa Y9Y8 IMG1
		movq	[rsi+rax-16],mm0
		movq	[rsi+rax- 8],mm1
		add		eax,16
		cmp		ebx, eax
	...
	four:
		test	ebx, 4
		jz		zero
		movd	mm0,[rdi+rax-16]  // ____ ____ y3y2 y1y0 img2
		movd	mm1,[rsi+rax-16]  // ____ ____ Y3Y2 Y1Y0 IMG1
		pavgb	mm0,mm1
		movd	[rsi+rax-16],mm0
		align 16
	zero:
