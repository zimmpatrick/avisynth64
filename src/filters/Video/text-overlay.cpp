...
do32:
        sar ecx, 1
        sub rsi, rcx  ; src - 12*pitch - 1
        lea rdi,[rsi+rdx*2]

        mov eax, [rsi]  ; repeat 32 times
        mov ecx, [rsi+rdx]
        lea rsi,[rsi+rdx*4]
        or eax, [rdi]
        or ecx, [rdi+rdx]
...
{ 
	...
	__int64 _rowsize = (__int64) rowsize;
	__int64 _pitch1 = (__int64) pitch1;
	__int64 _pitch2 = (__int64) pitch2;
    ...
    __asm 
	{
		...
		add			rsi, _rowsize
		add			rdi, _rowsize
		xor			rax, rax      ; sum of squared differences low
		xor			rdx, rdx      ; sum of squared differences high
		pxor		mm7, mm7      ; sum of absolute differences
		pxor		mm6, mm6      ; zero
		pxor		mm5, mm5      ; sum of differences
	comp_loopy:
