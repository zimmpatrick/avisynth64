   ...
   CodeGenerator(false) //Josh: Codegenerator construtor takes arg x64
{
  ...
  if (vi.IsRGB()) {  // Generate MMX
	...
	{
		if (vi.IsRGB24())
			convert_rgb_asm = _dupl ? &ConvertRGB24toYUY2_dup_sub_asm : &ConvertRGB24toYUY2_sub_asm;
		else
			convert_rgb_asm = _dupl ? &ConvertRGB32toYUY2_dup_sub_asm : &ConvertRGB32toYUY2_sub_asm;
	}
	else
	{
		if (vi.IsRGB24())
			convert_rgb_asm = _dupl ? &ConvertRGB24toYUY2_dup_asm : &ConvertRGB24toYUY2_asm;
		else
			convert_rgb_asm = _dupl ? &ConvertRGB32toYUY2_dup_asm : &ConvertRGB32toYUY2_asm;
	}*/
  }
                                       ...
                                       int pitch1Y, int pitch1UV, int pitch2, int width, int height)
{
	...
	yloop:
		...
		align 16
	xloop:
		movq		mm1,[rdx+rax]       ;uUuUuUuU
		movq		mm2,[rsi+rax]       ;vVvVvVvV
		pand		mm1,mm7             ;.U.U.U.U
		psllw		mm2,8               ;V.V.V.V.
		movq		mm0,[rcx+rax]       ;YYYYYYYY
		por			mm1,mm2             ;VUVUVUVU
		movq		mm3,mm0
		punpcklbw	mm0,mm1             ;VYUYVYUY
		add			eax,8
		punpckhbw	mm3,mm1             ;VYUYVYUY
		movq		[rdi+rax*2-16],mm0  ;store
		cmp			eax,width
		movq		[rdi+rax*2-8],mm3   ;store
		jl			xloop
