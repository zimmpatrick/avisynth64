...
double TDStretch3DNow::calcCrossCorrStereo(const float *pV1, const float *pV2) const
{
    ...
    */
    ...
    loop1:
        movq    mm1, [rax]
        prefetch [rax + 32]     // give a prefetch hint to CPU what data are to be needed soonish
        pfmul   mm1, [rbx]
        prefetch [rbx + 64]     // give a prefetch hint to CPU what data are to be needed soonish
...
void FIRFilter3DNow::setCoefficients(const float *coeffs, uint newLength, uint uResultDivFactor)
{
    ...
    filterCoeffsAlign = (float *)(((size_t)filterCoeffsUnalign + 15) & (size_t)-16);
...
uint FIRFilter3DNow::evaluateFilterStereo(float *dest, const float *src, uint numSamples) const
{
    ...
    */
    ...
    loop1:
        // "outer loop" : during each round 2*2 output samples are calculated
        prefetch  [rbx]                 // give a prefetch hint to CPU what data are to be needed soonish
        prefetch  [filterCoeffsLocal]   // give a prefetch hint to CPU what data are to be needed soonish

        mov     esi, ebx
        mov     rdi, filterCoeffsLocal
        pxor    mm0, mm0
        pxor    mm1, mm1
        mov     ecx, lengthLocal
    ...
    loop2:
        ...
        movq    mm2, [rdi]
        movq    mm3, mm2
        prefetch  [rdi + 32]     // give a prefetch hint to CPU what data are to be needed soonish
        pfmul   mm2, [rsi]
        prefetch  [rsi + 32]     // give a prefetch hint to CPU what data are to be needed soonish
        pfmul   mm3, [rsi + 8]
