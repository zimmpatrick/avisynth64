...
uint detectCPUextensions(void)
{
    ...
    _asm 
    {
        ...
        mov     eax,dword ptr [rsp] ; load eax from stack (with eflags)
        mov     ecx, eax            ; save the original eflags values to ecx
        xor     eax, 0x00200000     ; toggle bit 21
        mov     dword ptr [rsp],eax ; store toggled eflags to stack
        popf                        ; load eflags from stack

        pushf                       ; save updated eflags to stack
        mov     eax,dword ptr [rsp] ; load eax from stack
        popf                        ; pop stack to restore stack pointer
